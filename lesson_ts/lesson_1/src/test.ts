...
const objFromStorage2 = JSON.parse(localStorage.getItem("data") ?? "{}");
...
type product = [name: string, price: number, quantity: number, cost: number];

function createProduct(num: number): product {
  const name = prompt(`Назва товару ${num}:`) ?? "";
  const price = Number(prompt(`Ціна товару ${num}:`) ?? 0);
  const quantity = Number(prompt(`Кількість товару ${num}:`) ?? 0);
...
function startTask3() {
  if (!confirm("Почати задачу 3?")) return;
...
btn3?.addEventListener("click", startTask3);
...
function checkUserDay(dayProps: string | number): string {
...
if (btnTask4) {
  btnTask4.addEventListener("click", () => {
    const input = prompt("Введіть номер або назву дня тижня")
      ?.toLowerCase()
      .trim();
...
let countNum = 0;
let countOk = 0;
let randomNumOrOk: number | string;
...
if (resNum) resNum.textContent = `Кількість чисел: ${countNum}`;
if (resRow) resRow.textContent = `Кількість "Ok": ${countOk}`;
...
function convertVal(amount: number, currencyVal: string): string {
...
btn6?.addEventListener("click", () => {
  ...
  if (amountVal && currencyVal) {
    ...
    if (isNaN(amount)) {
      if (res6) res6.textContent = "Невірна сума";
    } else {
      const res = convertVal(amount, currencyVal.trim());
      if (res6) res6.textContent = res;
    }
  } else {
    if (res6) res6.textContent = "Введіть всі дані";
  }
});
