...
if (btnTask1 && task1) {
    btnTask1.addEventListener("click", () => {
        const userSeason = prompt("Введіть перший номер місяця пори року (3, 6, 9, 12):");
...
function searchLastDigitOfSymbolNumber(num) {
...
function getSeason(month) {
    ...
    if (typeof month === "string") {
        if (winterNames.includes(month))
            return "Зима";
        if (springNames.includes(month))
            return "Весна";
        if (summerNames.includes(month))
            return "Літо";
        if (autumnNames.includes(month))
            return "Осінь";
...
if (btnTask3 && task3) {
    btnTask3.addEventListener("click", () => {
        const userNum = prompt("Введіть назву місяця (наприклад, Січень, також з великої літери) або номер місяця (1-12):");
        if (userNum === null || userNum.trim() === "") {
...
function generateRandomDay(count) {
    ...
    const allDay = [...workDays, ...holidays];
    const res = [];
...
function getClosestPlace(lat, long, markers) {
    if (markers.length === 0)
        return null;
...
function generateArrAward(count) {
    const awards = [];
    for (let i = 0; i < count; i++) {
...
function awardDistribution(awards) {
    const distribution = {
        [Award.GOLD]: 0,
        [Award.SILVER]: 0,
        [Award.BRONZE]: 0,
